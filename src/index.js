  ...
  this.options = Object.assign(
    {
      ...
      marker: {
        image:
          "https://www.mapbox.com/help/demos/custom-markers-gl-js/mapbox-icon.png", // Default marker image to use as fallback
      },
      popup: {
        description: "<i>No description provided</i>", // Fallback description
      },
      style: {
        // Deafult style properties for the generated layers
        label: "{name}",
        labelSize: 10,
    ...
    function renderMarkers() {
      ...
      that._geojson.features.forEach(function (marker) {
        if (marker.geometry.type == "Point") {
          ...
          markerEl.style = `background-image:url('${
            marker.properties["marker-image"] || that.options.marker.image
          }')`;
          ...
          var popupHTML =
            (typeof marker.properties.title !== "undefined"
              ...
              : "") +
            (typeof marker.properties.image !== "undefined"
              ? `<img src='${marker.properties.image}' width=200 alt='${marker.properties.title}'>`
              : "") +
            (typeof marker.properties.description !== "undefined"
              ? `<p>${marker.properties.description}</p>`
              : `<p>${that.options.popup.description}</p>`) +
            (typeof marker.properties.website !== "undefined"
              ? `<a href='${marker.properties.website}' target='_blank' class='button'>クイズに挑戦する</a>`
              : `<a href='https://www.openstreetmap.org/?mlat=${marker.geometry.coordinates[1]}&mlon=${marker.geometry.coordinates[0]}' target='_blank' class='button'>クイズに挑戦する</a>`);
      ...
      if (!that.options.style.layers) {
        that.options.style.layers = buildStyleLayers(
          that._sourceName,
          that.options.style
        );
      }
      ...
      var mapStyle = addStyleLayers(
        style,
        that.options.style.layers,
        topRoadLayer
      );
...
MapboxMarkers.prototype._toggleLayers = function () {
  ...
  Array.from(document.getElementsByClassName("markerspec marker")).forEach(
    function (marker) {
